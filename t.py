...
class BackgroundThread(threading.Thread):
    ...
    def run(self) -> None:
        for i in self.watch_list:
            i.start()
        self.Recv.start()
        self.sched.add_job(self.GetList, 'interval', minutes=self.Minutes)
        self.sched.start()
        model_method_map = {
            "All_list": self.on_All_list,
            "ADD_FILE":self.on_ADD_FILE,
            "MODI_FILE":self.on_MODI_FILE,
            "RENAME_FILE": self.on_RENAME_FILE,
        ...
        self.Send.Broadcast("GEHA",self.HALO_data)
        self.GetList()
        while True:
            time.sleep(0.1)
            need_del = []
