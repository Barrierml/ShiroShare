            ...
            for filename in os.listdir(fn):
                if filename[0] != ".":
                    ...
                    if os.path.isdir(filepath):
                        filename += os.sep
                        contentFolders += """\n <tr><td valign="top"> <img src="http://www.apache.org/icons/dir.png" alt="[FOLDER]"> <a href="%s%s%s">%s</a> </td></tr>""" % (
                        Host, path, filename, filename)
                    else:
                        contentFileNames += """\n <tr><td valign="top"> <img src="http://www.apache.org/icons/generic.png" alt="[FILE]"> <a href="%s%s%s">%s</a> </td></tr> """ % (
                        Host, path, filename, filename)
            # Encode the output stream
            content = bytes("<h1>File System<h1>" + contentFolders + "</table>" + contentFileNames + "</table>",
                            "utf-8")
            self.send_header("content-type", "text/html")
